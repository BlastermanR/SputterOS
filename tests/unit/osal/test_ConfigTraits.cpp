...
TEST(CfgMaxSlotsPerCore, Default_Returns32) { EXPECT_EQ(CfgMaxSlotsPerCore<MinimalCfg>::value, 32u); }

TEST(CfgMaxSlotsPerCore, Override_ReturnsCustomValue) { EXPECT_EQ(CfgMaxSlotsPerCore<FullSchedulingCfg>::value, 64u); }

TEST(CfgMaxBackgroundTasks, Default_Returns16) { EXPECT_EQ(CfgMaxBackgroundTasks<MinimalCfg>::value, 16u); }
...
TEST(CfgCommsBudgetUs, Default_Returns1000) { EXPECT_EQ(CfgCommsBudgetUs<MinimalCfg>::value, 1000u); }

TEST(CfgCommsBudgetUs, Override_ReturnsCustomValue) { EXPECT_EQ(CfgCommsBudgetUs<FullSchedulingCfg>::value, 500u); }

TEST(CfgDiagsBudgetUs, Default_Returns10000) { EXPECT_EQ(CfgDiagsBudgetUs<MinimalCfg>::value, 10000u); }

TEST(CfgDiagsBudgetUs, Override_ReturnsCustomValue) { EXPECT_EQ(CfgDiagsBudgetUs<FullSchedulingCfg>::value, 5000u); }

TEST(CfgMinGapSliceUs, Default_Returns10) { EXPECT_EQ(CfgMinGapSliceUs<MinimalCfg>::value, 10u); }

TEST(CfgMinGapSliceUs, Override_ReturnsCustomValue) { EXPECT_EQ(CfgMinGapSliceUs<FullSchedulingCfg>::value, 50u); }

TEST(CfgMinSchedulePeriodUs, Default_Returns10) { EXPECT_EQ(CfgMinSchedulePeriodUs<MinimalCfg>::value, 10u); }
...
TEST(CfgStrictWCET, Default_ReturnsFalse) { EXPECT_FALSE(CfgStrictWCET<MinimalCfg>::value); }

TEST(CfgStrictWCET, Override_ReturnsTrue) { EXPECT_TRUE(CfgStrictWCET<FullSchedulingCfg>::value); }
...
TEST(IsScheduledTask, Positive_DetectsSubclass) { EXPECT_TRUE(IsScheduledTask<StubScheduled>::value); }

TEST(IsScheduledTask, Negative_RejectsUnrelated) { EXPECT_FALSE(IsScheduledTask<PlainClass>::value); }

TEST(IsBackgroundTask, Positive_DetectsSubclass) { EXPECT_TRUE(IsBackgroundTask<StubBackground>::value); }

TEST(IsBackgroundTask, Negative_RejectsUnrelated) { EXPECT_FALSE(IsBackgroundTask<PlainClass>::value); }
